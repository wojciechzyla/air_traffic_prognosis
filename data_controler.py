...
# -*- coding: utf-8 -*-
import json
import destination_data as dd
import origin_data as od
from runways_coordinates import get_runways_and_distance
...
for arpt in airports_list:
    ...
    for dest in destination_data:
        ...
        try:
            ...
            continue
        try:
            with open("json/flights1.json", "r+") as file:
                data = json.load(file)
                data += new_flights
                file.seek(0)
                json.dump(data, file)
        except json.JSONDecodeError:
            with open("json/flights1.json", "w") as file:
